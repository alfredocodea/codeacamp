...
p average_grade(grades_1) {|grades|
  ...
  time = Time.now  # time.now es para cubrir un tiempo que empieza y acaba, es lo que dura.
  average = ((grades.inject(0){ |result, grade| result + grade})/grades.length).round(1)  # el metodo inject usando un block
